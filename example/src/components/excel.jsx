...
const Excel = ({ systemItems, setSystemItems, handleNext }) => {
      ...
      const [items, setItems] = useState(() => {
          const savedSystemItems = localStorage.getItem('systemItems');
          return savedSystemItems ? JSON.parse(savedSystemItems) : systemItems;
      });
      ...
      const handleReset = () => {
        localStorage.removeItem('systemItems');
        localStorage.removeItem('addOnItems');
        setItems([ {
            selectedComponent: '',
            selectedSystem: "",
            selectedSize: '',
                    ...
                    <div style={{ marginBottom: '16px' }}>
                        ...
                        >
                            ...
                            {systems.map((system, idx) => (
                                <option key={idx} value={system}>{system}</option>
                            ))}
                        </select>
                    </div>
                    ...
                    {item.selectedSharing && (
                        <div style={{ marginBottom: '16px' }}>
                            ...
                            >
                                ...
                                {item.dimensions.map((dimension, idx) => (
                                    <option key={idx} value={dimension}>{dimension}</option>
                                ))}
                            </select>
                        </div>
                    )}
